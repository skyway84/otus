...
Блокировки:
SELECT * FROM locks_v WHERE pid = 6801;
  pid  |   locktype    |  lockid  |       mode       | granted 
-------+---------------+----------+------------------+---------
 12658 | relation      | accounts | RowExclusiveLock | t        -   Транзакция успешно вызвала блокировку строки
 12658 | transactionid | 749      | ExclusiveLock    | t        -   Транзакция заблокировала свой ID
...
Блокировки:
locks=# SELECT * FROM locks_v WHERE pid = 12490;
  pid  |   locktype    |   lockid   |       mode       | granted 
-------+---------------+------------+------------------+---------
 12490 | relation      | accounts   | RowExclusiveLock | t
 12490 | transactionid | 750        | ExclusiveLock    | t
 12490 | tuple         | accounts:1 | ExclusiveLock    | t
 12490 | transactionid | 749        | ShareLock        | f
...
Блокировки:
locks=*# SELECT * FROM locks_v WHERE pid = 12593;
  pid  |   locktype    |   lockid   |       mode       | granted 
-------+---------------+------------+------------------+---------
 12593 | relation      | accounts   | RowExclusiveLock | t
 12593 | transactionid | 751        | ExclusiveLock    | t
 12593 | tuple         | accounts:1 | ExclusiveLock    | f


Сессия 1:
Обращаемся к pg_lock
...
В журнале Postgresql появились записи:
